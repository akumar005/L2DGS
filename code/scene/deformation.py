...
class Deformation(nn.Module):
    ...
    def create_net(self):
        ...
        #self.shs_deform_ill = nn.Sequential(nn.ELU(),nn.Linear(self.W,self.W),nn.ELU(),nn.Linear(self.W, 16),nn.ReLU())
    ...
    def forward_dynamic(self,rays_pts_emb, scales_emb, rotations_emb, opacity_emb, shs_emb, time_feature, time_emb):
        ...
        #dshs = self.shs_deform_ill(hidden).reshape([shs_emb.shape[0],16,1])
    ...
    def get_mlp_parameters(self):
        ...
        for name, param in self.named_parameters():
            if not any(substring in name for substring in ["grid"]):
    ...
    def get_grid_parameters(self):
        parameter_list = []
        for name, param in self.named_parameters():
            if any(substring in name for substring in ["grid"]):
                parameter_list.append(param)
        return parameter_list
...
class deform_network(nn.Module):
    def __init__(self, args) :
        ...
        self.shs_deform_net = nn.Sequential(nn.ELU(),nn.Linear(128, 128),nn.ELU(),nn.Linear(128, 16),nn.ReLU())
    ...
    def forward_dynamic(self, point, scales=None, rotations=None, opacity=None, shs=None, times_sel=None):
        ...
        means3D, scales, rotations, opacity, shs, hidden = self.deformation_net( point_emb,
                                                  scales_emb,
                                                rotations_emb,
                                                opacity,
                                                shs,
        ...
        return means3D, scales, rotations, opacity, shs , self.shs_deform_net(hidden)


    def get_mlp_parameters(self):
        return self.deformation_net.get_mlp_parameters() + list(self.timenet.parameters()) + list(self.shs_deform_net.parameters())
    def get_grid_parameters(self):
...
# def dual_threshold(matrix, a=-0.5, b=0.5):
#     """
#     Custom ReLU function that sets values in the range [a, b] to 0.
...
#     Returns:
#         torch.Tensor: Output tensor with values in [a, b] set to 0.
#     """
#     lower_mask = matrix < a
#     upper_mask = matrix > b
#     outside_range = lower_mask | upper_mask  # Logical OR
#     return matrix * outside_range  # Retain values outside [a, b]
